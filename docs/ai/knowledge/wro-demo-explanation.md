...
Local Ollama allows the demo to run on a WRO laptop without depending on cloud connectivity and keeps response data on the machine. Microphone input, local Whisper transcription, the “Hey Angklobot” phrase, text-to-speech, and local MIDI conversion are implemented software features. Camera-based presence detection and complete physical-hardware validation remain future work.
